...
mod host_tests {
    ...
    fn sccache_below_the_uds_floor_is_rejected() {
        ...
        if crate::toolchain::managed_tool::sccache().is_some() {
            // The pinned artifact repairs a stale PATH copy the package
    ...
    fn missing_without_installer_uses_the_release_artifact() {
        ...
        // Wherever upstream publishes an artifact for this platform the
        // pinned build is the repair — a distribution package can lag the
        // floor. Only an artifact-less host stays manual.
        if crate::toolchain::managed_tool::sccache().is_some() {
            assert!(
                matches!(
                    result,
            ...
            assert!(
                matches!(result, Err(ToolchainError::Unfixable(_))),
                "missing sccache on an artifact-less host without an installer must be unfixable: {result:?}"
            );
        }
    }
